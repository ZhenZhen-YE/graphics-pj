...
//  gl
//
//  Created by 叶蓁蓁 on 2019/12/26.
//  Copyright © 2019 叶蓁蓁. All rights reserved.
//
...
const char* skyFragShader = GLSL(
  ...
  void main()
  {
    ...
    float mu = dot(normalize(pos), normalize(pos))/5;
    vec3 extinction = mix(exp(-exp(-((pos.y) * (exp(-pos.y * 16.0) + 0.1) / 80.0) / Br) * (exp(-pos.y * 16.0) + 0.1) * Kr / Br) * exp(-pos.y * exp(-pos.y * 8.0 ) * 4.0) * exp(-pos.y * 2.0) * 4.0, vec3(1.0 ) * 0.2, -0.2 + 0.5)*3;
    color.rgb = 3.0 / (8.0 * 3.14) * (1.0 + mu * mu) * (Kr + Km * (1.0 - g * g) / (2.0 + g * g) / pow(1.0 + g * g - 2.0 * g * mu, 1.5)) / (Br + Bm) * extinction/4;

    // Cirrus Clouds
    float density = smoothstep(1.0 - cirrus, 1.0, fbm(pos.xyz / pos.y * 2.0 + time * 0.05)) * 0.3;
    color.rgb = mix(color.rgb, extinction * 10.0, density * max(pos.y, 0.0)/3);
...
const char *postFragShader = GLSL(
  ...
  void main()
  {
    ...
    for (int i = -2; i < 3; i++)
    {
      for (int j = -2; j < 3; j++)
      {
        occlusion += 1.0 / (1.0 + pow(10.0 * min(depth - texture(tex[1], UV + vec2(i, j) * r).r, 0.0), 2.0)) / 14.0;
      }
    }
    color.rgb *= occlusion;
...
float floorCoords[] = {
   3.0f, -0.1f, -3.0f, 0.5f, 0.0f, 0.0f,
  -3.0f, -0.1f, -3.0f, 0.0f, 0.0f, 0.0f,
   3.0f, -0.1f,  3.0f, 0.5f, 0.5f, 0.0f,
  -3.0f, -0.1f,  3.0f, 0.0f, 0.5f, 0.0f,
};
...
entity makeEntity(scene *s, const char* vs, const char* fs, int texcount, char textures[][80],
...
int main()
{
  ...
  makeEntity(&s, postVertShader, postFragShader, 0, NULL, NULL, 4, 0, 6, 0, 800, 600);
